...
**Follow-on (same change set):** investigation surfaced that `waitUntil: 'networkidle'` is officially discouraged by Playwright ([Pavel Feldman, microsoft/playwright#22897](https://github.com/microsoft/playwright/issues/22897)) and routinely hangs on real-world pages with analytics/polling. Both reference AI-browser tools — [browser-use](https://github.com/browser-use/browser-use) and [agent-browser](https://github.com/vercel-labs/agent-browser) — default to `'load'`. Switched `navigate()` accordingly, which makes the fatal behaviour above robust (a timeout now means "page genuinely broken", not "tracker pinging in a loop"). Also hardcoded `observe()`'s post-snapshot networkidle wait to 2s (was using the full `--network-timeout`, burning up to 30s per turn on chatty pages); the wait is silent-soft-fail so the cap stays internal — not user-tunable. As a result, `--network-timeout` now governs `page.goto()` only.
